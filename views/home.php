...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge.online { background: rgba(0,255,136,.1); color: var(--accent); }
        ...
        .server-table tr:not(:last-child) td { border-bottom: 1px solid var(--border); }
        .server-table td { padding: .35rem .5rem; vertical-align: top; }
        .server-table td:first-child { color: var(--muted); width: 38%; word-break: break-all; }
        .server-table td:last-child  { color: var(--accent2); word-break: break-all; }
...
async function runPost() {
    ...
    try {
        const t0 = performance.now();
        const r  = await fetch('/test-echo', { method: 'POST', headers, body });
        const ms = performance.now() - t0;
...
async function runUpload() {
    ...
    if (!files.length) { el.innerHTML = '<span class="warn">Dosya seçilmedi</span>'; return; }
    ...
    try {
        const t0 = performance.now();
        const r  = await fetch('/test-upload', { method: 'POST', body: fd });
        const ms = performance.now() - t0;
        const txt = await r.text();
        el.innerHTML = `<span class="ok">HTTP ${r.status} — ${ms.toFixed(1)}ms</span>\n\n${escHtml(txt)}`;
    } catch (e) {
